...
@export var camp_piece_data: PieceData
...
var _awaiting_camp_placement := false
...
func begin_camp_placement() -> void:
	_awaiting_camp_placement = true

func _on_slot_clicked(slot: WorldSlot) -> void:
	if not _awaiting_camp_placement:
		return
	
	_awaiting_camp_placement = false
	
	var data := camp_piece_data.duplicate() as PieceData
	slot.set_piece(data)
